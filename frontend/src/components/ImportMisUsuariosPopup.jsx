...
import { MdUploadFile, MdTableChart, MdError } from 'react-icons/md';
import { FaTrash } from 'react-icons/fa';
import CloseIcon from '@assets/XIcon.svg';
import QuestionIcon from '@assets/QuestionCircleIcon.svg';

// Roles disponibles para encargados de carrera (sin administrador ni encargado_carrera)
const ROLES_MIS_USUARIOS = [
  "tutor",
  "tutorado", 
  "usuario"
];
...
/**
 * Componente de importación de usuarios para MisUsuarios
 * Basado en la arquitectura moderna de ImportUsersPopup
 */
const ImportMisUsuariosPopup = ({ show, setShow, onFile, loading = false }) => {
  // Estados principales
  const [fileName, setFileName] = useState('');
  const [tableData, setTableData] = useState([]); // Un solo estado para los datos
  ...
  // Función para actualizar celda individual
  ...
  const deleteRow = useCallback((rowIndex) => {
    setTableData(prevData => prevData.filter((_, idx) => idx !== rowIndex));
    setImportedRows(prev => prev.filter(idx => idx !== rowIndex).map(idx => idx > rowIndex ? idx - 1 : idx));
    setRowErrors(prev => {
  ...
  // Función para cerrar archivo sin cerrar popup
  ...
  const pending = total - imported - errors;
      ...
      <div className="relative bg-white shadow-2xl w-full max-w-7xl h-[95vh] p-0 animate-fade-in flex flex-col rounded-2xl border border-gray-200">
        ...
        <div className="flex items-center justify-between px-6 py-3 border-b border-gray-200 bg-gradient-to-r from-[#4EB9FA]/5 to-[#5EBFFA]/5 rounded-t-2xl flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-[#2C3E50]">Importar Usuarios</h2>
              <p className="text-gray-600 text-sm">Carga y gestiona usuarios desde archivos Excel</p>
            </div>
          </div>
          <button
            ...
            <div className="flex flex-col items-center justify-center flex-1 gap-6">
              ...
              <div className="text-center max-w-2xl">
                <h3 className="text-2xl font-semibold text-[#2C3E50] mb-3">
                  Selecciona tu archivo de usuarios
                </h3>
                      ...
                      <div className="grid gap-3 max-w-2xl mx-auto">
                        <div className="bg-red-50 border border-red-200 rounded-lg p-3">
                          ...
                          <div className="space-y-1">
                            <div className="text-sm text-red-600">• Nombre Completo</div>
                            <div className="text-sm text-red-600">• Rut</div>
                            <div className="text-sm text-red-600">• Correo</div>
                            <div className="text-sm text-red-600">• Contraseña</div>
                            <div className="text-sm text-red-600">• Carrera</div>
                          </div>
                        </div>
                      </div>
            ...
            <div className="flex flex-col h-full space-y-4">
              ...
              <div className="bg-gradient-to-r from-[#4EB9FA]/5 to-[#5EBFFA]/5 rounded-xl p-4 border border-[#4EB9FA]/20 flex-shrink-0">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    <div className="p-2 bg-[#4EB9FA]/10 rounded-xl">
                      <MdTableChart size={24} className="text-[#4EB9FA]" />
                    </div>
                    <div>
                  ...
                  <div className="flex flex-wrap gap-3">
                    <div className="flex items-center gap-2 bg-green-100 px-3 py-2 rounded-lg">
                      ...
                      <span className="text-green-800 font-medium text-sm">Importados: {imported}</span>
                    </div>
                    <div className="flex items-center gap-2 bg-red-100 px-3 py-2 rounded-lg">
                      <span className="w-3 h-3 bg-red-500 rounded-full"></span>
                      <span className="text-red-800 font-medium text-sm">Con errores: {errors}</span>
                    </div>
                        ...
                        {columns.map(col => (
                          <th key={col} className="px-4 py-3 text-left font-semibold text-gray-800 border-b border-gray-200 min-w-[140px] text-sm">
                            <div className="flex items-center gap-2">
                              ...
                              {['nombreCompleto', 'rut', 'email', 'password'].includes(col) && (
                                  ...
                                  <div className="flex flex-col gap-1">
                                    {field === 'rol' ? (
                                      ...
                                      <input
                                        ...
                                        placeholder={
                                          field === 'carrera' ? 'Nombre de carrera' :
                                          field === 'nombreCompleto' ? 'Nombre completo del usuario' :
...
ImportMisUsuariosPopup.propTypes = {
...
export default ImportMisUsuariosPopup;